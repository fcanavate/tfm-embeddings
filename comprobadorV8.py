...
base_verb_pd=pd.read_excel("base_verb_v1.xlsx",names=['index_verb','verbostd','verbo'])
base_verb_pd.index = np.arange(1, len(base_verb_pd)+1)
base_noun_pd=pd.read_excel("base_noun_v1.xlsx",names=['index_noun','nombrestd','nombre'])
base_noun_pd.index = np.arange(1, len(base_noun_pd)+1)
coord_nounsuj_verb_pd=pd.read_excel("coord_nounsuj_verb_v1.xlsx",names=['noun_suj_index','verbindex','coordenada'])
...
def vector_verb_suj_index(index):
    index_verb=index
    vectorverb=coord_nounsuj_verb_matrix.getcol(index_verb)
    vector_verb_dense=vectorverb.todense()
    return(vector_verb_dense)
...
for i in range(1,5000):
    ...
    if i%5==0:
        ...
        nombres_global_red.to_csv("nombres_global_red.csv",encoding="utf_8_sig")
        nombres_global_normal.to_csv("nombres_global_normal.csv",encoding="utf_8_sig")
#nombresnozeros.to_csv("nombrenozeros.csv",encoding='utf_8_sig')
nombres_global_red.to_csv("nombres_global_red.csv",encoding="utf_8_sig")
nombres_global_normal.to_csv("nombres_global_normal.csv",encoding="utf_8_sig")